...
rename_report_files() {
    ...
    report_file=$(find "$folder" -maxdepth 1 -type f -name "*.html" ! -name "report.html" -exec du -b {} + | sort -nr | head -n 1 | awk '{print $2}')
...
git add .
git commit -m "Recursively renamed report files inside all nested folders"
git push origin main

echo "✅ Report file renaming completed in all nested folders!"
