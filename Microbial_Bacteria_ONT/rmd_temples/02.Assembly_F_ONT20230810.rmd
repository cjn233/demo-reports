...
&emsp;&emsp;从各样品质控后的有效数据出发，使用两款组装软件并行组装，依据组装质量选取最优组装结果（n50最长）。两款软件分别为：Flye软件(https://github.com/fenderglass/Flye，version：2.9-b1768)、Canu软件(https://github.com/marbl/canu/ ,version：2.0)，分别进行基因组组装，得到能反映样品基因组基本情况的初步的组装结果，然后使用 Racon（version：1.4.13）软件基于三代测序数据对拼接结果进行三轮纠错后，再进行三轮二代 reads 的 Pilon（version：1.22）纠错，得到最终的组装结果，组装结果如下表所示：


<p class='mark'>表<font face="Arial">表 2-3 样品基因组组装结果统计</p>
